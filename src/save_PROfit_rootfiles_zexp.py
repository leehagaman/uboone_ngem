...
#     isdata, isext, isdirt, isnuwro (filetype flags)
#     <NET_WEIGHT_COL>               (raw open-data net weight)
#     net_weight_nuwro               (NuWro-normalized prediction/pseudo-data weight;
...
def write_withspline_root(mc_df, data_df, spline_path, output_path, batch_size=131_072):
    ...
    for c in out_cols:
        ...
        if kind == "list":
            ...
            buffers[c][0] = 1.0 if (
                c in spline_schema.names()
                or c in ("fraction_with_spline_weights",
                         "spline_processed_fraction_weight")
            ) else np.nan
    ...
    # NuWro is excluded from the open-data weighting configuration, so its
    # open-data weight is null. Write a finite zero: otherwise a PROfit branch
    # can evaluate a rejected event as 0 * NaN and poison the histogram.
    net = (
        data_df[NET_WEIGHT_COL]
        .fill_null(0.0)
        .fill_nan(0.0)
        .to_numpy()
        .astype(np.float64)
...
def build_minimal_df(training):
    ...
    # Drop raw 1g overlays, which are retained through their reweighted filetypes.
    # NuWro is retained below as a data-role sample with its own normalization.
    pred = merged.filter(~pl.col("filetype").is_in(
        ["data", "nuwro_fake_data", "isotropic_one_gamma_overlay",
         "delete_one_gamma_overlay", "fullosc_overlay"]))
    data = merged.filter(pl.col("filetype").is_in(["data", "nuwro_fake_data"]))
    ...
    combined = combined.with_columns([
        # Preserve this configuration's weight for both prediction and NuWro.
        # Filetypes excluded from the configuration receive a finite zero.
        pl.col(NUWRO_WEIGHT_COL)
        .fill_null(0.0)
        .fill_nan(0.0)
...
def save_nominal(training, output_dir):
    ...
    data_role_filetypes = ["data", "ext", "nuwro_fake_data"]
    mc_df = minimal_df.filter(~pl.col("filetype").is_in(data_role_filetypes)).collect(engine="streaming")
    data_df = minimal_df.filter(pl.col("filetype").is_in(data_role_filetypes)).collect(engine="streaming")
    print(f"  {mc_df.height} MC events, {data_df.height} data/ext/NuWro events")
